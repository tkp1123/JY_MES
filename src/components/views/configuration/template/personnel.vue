  ...
  <div>
    ...
    <el-row :gutter="20" class="pdbt10 pdr10 pdl10">
      <el-table :data="tableData" border stripe style="width: 100%">
        <el-table-column prop="operatorId" label="ID"></el-table-column>
        <el-table-column prop="nick" label="登录ID"></el-table-column>
...
<script>
import { operator_api } from '@/utils/api'
...
export default {
  ...
  methods: {
    getOrder() {
      operator_api({ pageIndex: 1, pageSize: 10, sortDirection: 'DESC' }).then(
        (res) => {
          if (res.resultCode == 0) {
    ...
    formatLastLoginTime(row, column, cellValue) {
      if (!cellValue) return ''
